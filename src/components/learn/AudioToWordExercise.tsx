...
import { resolveAudioToWord } from "@/lib/curriculum/wordAdapter.ts";
...
export function AudioToWordExercise({ exercise, bundle, onResult, locked }: ExerciseViewProps) {
  const [wrongId, setWrongId] = useState<string | null>(null);
  const resolved = resolveAudioToWord(bundle, exercise);
  ...
  if (!resolved) {
    return <p className="text-center font-display text-2xl font-extrabold">تَعَذَّرَ تَحْمِيلُ الْكَلِمَة.</p>;
  }
  ...
  return (
    <div className="animate-pop text-center">
      <p className="mb-5 font-display text-2xl font-extrabold md:text-3xl">اِخْتَرِ الْكَلِمَةَ</p>
      ...
      <div className={cn("mx-auto grid max-w-lg gap-4", displayChoices.length > 2 ? "grid-cols-3" : "grid-cols-2")}>
        {displayChoices.map((choice) => (
          ...
          >
            <span className="arabic-letter text-5xl md:text-6xl">{choice.displayText}</span>
          </button>
        ))}
      </div>
    </div>
