...
import { resolvePictureToWord } from "@/lib/curriculum/wordAdapter.ts";
...
export function PictureToWordExercise({ exercise, bundle, onResult, locked }: ExerciseViewProps) {
  const [wrongId, setWrongId] = useState<string | null>(null);
  const resolved = resolvePictureToWord(bundle, exercise);
    ...
    <div className="animate-pop text-center">
      ...
      <div className="mx-auto mb-8 grid min-h-40 max-w-xs place-items-center rounded-[2rem] border-4 border-sun bg-sun/20 px-6 shadow-chunky-xs">
        <span className="text-8xl" aria-hidden>
          {resolved.visual.kind === "emoji" ? resolved.visual.emoji : "📷"}
        </span>
      </div>
